...
savepath1      <- paste0("F:/experiment/ML 2021/Result/",paste0("BOKS-r-",Dataset[d_index],".txt"))
savepath2      <- paste0("F:/experiment/ML 2021/Result/",paste0("BOKS-r-all-",Dataset[d_index],".txt"))
...
for(re in 1:reptimes)
{
  ...
  for (i in 1:length_tr)
  {
    ...
    if(sum(k)<para1_setting$B)
    {
      ...
      tem_svpara[k[It]]   <- -para1_setting$lambda*sign(sum-ylabel[order[i]])/p[It]
